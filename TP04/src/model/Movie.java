...
import criptografia.ControllerCriptografia;
...
public class Movie {
    ...
    /**
     * Escreve o campo overview criptografado no fluxo de saída de dados.
     * @param dos Fluxo de saída de dados
     * @param str String a ser escrita
     * @throws IOException Se ocorrer um erro de escrita
     */
    private void writeOverview(DataOutputStream dos, String str) throws IOException {
        byte[] utf8Bytes = str.getBytes("UTF-8");
        byte[] criptografado = ControllerCriptografia.criptografar(utf8Bytes);
        dos.writeInt(criptografado.length); // Escreve o tamanho da string
        dos.write(criptografado); // Escreve os bytes da string
    }
    ...
    /**
     * Lê o campo overview do fluxo de entrada de dados e descriptografa.
     * @param dis Fluxo de entrada de dados
     * @return String lida
     * @throws IOException Se ocorrer um erro de leitura
     */
    private String readOverview(DataInputStream dis) throws IOException {
        int length = dis.readInt(); // Lê o tamanho da string
        byte[] dados = new byte[length];
        dis.readFully(dados); // Lê os bytes da string
        byte[] descriptografado = ControllerCriptografia.descriptografar(dados);
        return new String(descriptografado, "UTF-8"); // Converte para string corretamente
    }
    ...
    public boolean equals(Object o) {
        ...
        Movie movie = (Movie) o;
        return id == movie.id; // Compara por ID (ou outro campo único)
    }
