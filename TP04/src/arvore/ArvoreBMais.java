 ...
 * ARVORE B+ 
 * 
 * Código baseado no do Prof. Marcos Kutova
 * Adições dos métodos readPK, readPK1 e getPosition
 */
package arvore;
...
public class ArvoreBMais<T extends RegistroArvoreBMais<T>> {
    ...
    // Método adicionado ao código do professor Kutova que faz busca recursiva
    // por um elemento a partir da chave primária. Este método invoca
    // o método recursivo readPK1, passando a raiz como referência.
    // O método retorna o elemento encontrado ou null se não existir
    public T readPK(T elem) throws Exception {
        // Recupera a raiz da árvore
        long raiz;
        arquivo.seek(0);
        raiz = arquivo.readLong();
    ...
    // Busca recursiva. Este método recebe a referência de uma página e busca
    // pela chave na mesma. A busca continua pelos filhos, se houverem.
    private T readPK1(T elem, long pagina) throws Exception {
        // Se chegou a uma página inexistente, retorna null
        if (pagina == -1) {
            return null;
        }
        ...
        if (i < pa.elementos.size() && pa.filhos.get(0) == -1 && elem.compareTo(pa.elementos.get(i)) == 0) {
            ...
            return pa.elementos.get(i);
        }

        // Se chegou ao final de uma folha, verifica se a chave está na próxima folha
        else if (i == pa.elementos.size() && pa.filhos.get(0) == -1) {
            // Testa se há uma próxima folha
            if (pa.proxima == -1) {
                return null;
            }
            ...
            // Verifica se a chave é a primeira da próxima folha
            i = 0;
            if (elem.compareTo(pa.elementos.get(i)) == 0) {
                return pa.elementos.get(i);
            } else {
        ...
        // Continua a busca recursiva pela árvore
        if (i == pa.elementos.size() || elem.compareTo(pa.elementos.get(i)) <= 0)
            return readPK1(elem, pa.filhos.get(i));
        else
            return readPK1(elem, pa.filhos.get(i + 1));
    }

    // Metodo adicionado ao código do professor Kutova para retornar a posição guardada pelo 
    // registro de índice (atributo pos do RegistroArvore) ao buscá-lo pelo id
    public long getPosition(T elem) throws Exception{
		...
		T registro = readPK(elem);

        if(registro == null) return -1;

		return registro.getPos(); // retorna o atributo pos do registro
	}
    ...
    private boolean create1(long pagina) throws Exception {
        ...
        // A inclusão pode ter sido resolvida porque o elemento já existia
        // (inclusão inválida)
    ...
    private boolean delete1(T elem, long pagina) throws Exception {
        ...
        // Encontra a página em que o elemento está presente
        ...
        // Elemento encontrado em uma folha
        if (i < pa.elementos.size() && pa.filhos.get(0) == -1 && elem.compareTo(pa.elementos.get(i)) == 0) {
