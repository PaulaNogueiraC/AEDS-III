...
public class HashExtensivel<T extends RegistroHashExtensivel<T>> {
  ...
  public class Cesto {
    ...
    public Cesto(Constructor<T> ct, int qtdmax, int pl) throws Exception {
      construtor = ct;
      if (qtdmax > 500) // Nossa base de dados possui 10000 registros, logo o bucket pode conter no máximo 5% disso segundo a especificação do TP
        throw new Exception("Quantidade máxima de 500 elementos");
      if (pl > 127)
  ...
  public long getPosition(int chave) throws Exception {
    T registroEncontrado = this.read(chave);

    // Se o elemento foi encontrado, retorna sua posição usando getPos()
    if (registroEncontrado != null) {
      return registroEncontrado.getPos();
    }
